...
function _smoothScale ( curScale, delta ) {
    var scale = curScale;
    scale = Math.pow( 2, delta * 0.002) * scale;
    return scale;
}

function _formatFrame ( frame, frameRate ) {
    ...
    return text +
        ...
        Fire.padLeft(frame % frameRate, decimals, '0');
}
...
Polymer({
    ...
    setScaleH: function ( lods, minScale, maxScale, type ) {
        ...
        if ( type === 'frame' ) {
            this.hformat = function ( frame ) {
                return _formatFrame( frame, 60.0 );
            };
        }
    ...
    scaleAction: function ( event ) {
        ...
        if ( changeX && this.hticks ) {
            newScale = _smoothScale(this.xAxisScale, event.wheelDelta);
            this.xAxisScaleAt ( event.offsetX, newScale );
        }

        if ( changeY && this.vticks ) {
            newScale = _smoothScale(this.yAxisScale, event.wheelDelta);
    ...
    _updateGrids: function () {
        ...
        // draw label
        if ( this['show-label-h'] || this['show-label-v'] ) {
            ...
            // draw hlabel
            if ( this['show-label-h'] && this.hticks ) {
                ...
                for ( j = 0; j < ticks.length; ++j ) {
                    ...
                    labelEL.style.right = '';
                    labelEL.style.top = '';
                    this.$.hlabels.appendChild(labelEL);
                }
            }

            // draw vlabel
            if ( this['show-label-v'] && this.vticks ) {
                ...
                for ( j = 0; j < ticks.length; ++j ) {
                    ...
                    labelEL.style.right = '';
                    this.$.vlabels.appendChild(labelEL);
                }
            }
        ...
        // DEBUG
        if ( this['show-debug-info'] ) {
            this.debugInfo.xAxisScale = this.xAxisScale.toFixed(3);
            this.debugInfo.xAxisOffset = this.xAxisOffset.toFixed(3);
            if ( this.hticks ) {
                this.debugInfo.xMinLevel = this.hticks.minTickLevel;
                this.debugInfo.xMaxLevel = this.hticks.maxTickLevel;
            }
            this.debugInfo.yAxisScale = this.yAxisScale.toFixed(3);
            this.debugInfo.yAxisOffset = this.yAxisOffset.toFixed(3);
            if ( this.vticks ) {
                this.debugInfo.yMinLevel = this.vticks.minTickLevel;
                this.debugInfo.yMaxLevel = this.vticks.maxTickLevel;
            }
        }
    },
